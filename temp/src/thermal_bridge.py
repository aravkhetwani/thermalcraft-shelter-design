...
def run_corner_bridge(
    ...
    try:
        # Theoretical 1D nominal flux far from corner: q_1D = k * (T_out - T_in) / t_w
        ...
        # 1. Reset and preprocessor
        mapdl.clear()
        mapdl.prep7()
        mapdl.et(1, config.element_type)
        mapdl.mp("KXX", 1, config.thermal_conductivity)

        # 2. Geometry: Create horizontal arm and vertical arm
        # Arm 1 (horizontal): X in [0, arm_length_x], Y in [0, wall_thickness]
        mapdl.blc4(0, 0, config.arm_length_x, config.wall_thickness)
        
        # Arm 2 (vertical): X in [0, wall_thickness], Y in [wall_thickness, arm_length_y - wall_thickness]
        mapdl.blc4(0, config.wall_thickness, config.wall_thickness, config.arm_length_y - config.wall_thickness)

        # Glue areas to form monolithic L-corner
        mapdl.aglue("ALL")
        ...
        # Outside hot boundary: Y=0 (bottom) and X=0 (left)
        mapdl.nsel("S", "LOC", "Y", 0)
        mapdl.nsel("A", "LOC", "X", 0)
        mapdl.d("ALL", "TEMP", config.outside_temperature)

        # Inside cool boundary:
        # Horizontal inner face: Y = wall_thickness, X in [wall_thickness, arm_length_x]
        ...
        # Vertical inner face: X = wall_thickness, Y in [wall_thickness, arm_length_y]
...
def _generate_corner_visualization(result: CornerBridgeResult, save_path: Optional[str] = None, show: bool = False):
    ...
    # Panel 2: Heat Flux Vector Quiver & Magnitude
    sc2 = ax2.scatter(
    ...
    if save_path:
        ...
        print(f"Corner bridge plot saved to: {save_path}")
