...
class LayerConfig:
    ...
    thermal_conductivity: float # Thermal conductivity k in W/(m·K)
...
def run_composite_wall(
    ...
    try:
        ...
        # Theoretical interface temperatures
        ...
        # 2. Define Materials and create rectangular areas
        layer_x_starts = []
        layer_x_mids = []
        x_cursor = 0.0
        ...
        # 3. Glue all areas together to enforce nodal continuity at interfaces
        mapdl.aglue("ALL")

        # 4. Assign material IDs to corresponding glued areas
...
def _generate_composite_visualization(result: CompositeWallResult, save_path: Optional[str] = None, show: bool = False):
    ...
    # Panel 1: 2D Temperature Contour with Layer Boundaries
    sc = ax1.scatter(
        result.x_coords,
        result.y_coords,
    ...
    # Draw vertical layer interface lines
    x_accum = 0.0
    ...
    # Panel 2: 1D Temperature Profile vs Theoretical Interface Points
    # Sample nodes near the middle height Y = height / 2
    ...
    # Plot theoretical interface points
    x_interfaces = [0.0]
    acc = 0.0
    ...
    # Draw vertical layer lines in 1D plot
    ...
    if save_path:
        ...
        print(f"Composite wall plot saved to: {save_path}")
