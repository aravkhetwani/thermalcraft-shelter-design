...
def run_convection_wall(
    ...
    try:
        # 1. Theoretical Resistance Calculation
        ...
        # 2. Reset and enter preprocessor
        mapdl.clear()
        mapdl.prep7()
        ...
        # 5. Apply Convective (Robin) Boundary Conditions using node surface loads
        # Outdoor Convection: X = 0
        mapdl.nsel("S", "LOC", "X", 0)
        mapdl.sf("ALL", "CONV", config.h_outdoor, config.outdoor_ambient_temp)

        # Indoor Convection: X = total_thickness
        ...
        # Extract simulated surface temperatures
...
def _generate_convection_visualization(result: ConvectionWallResult, save_path: Optional[str] = None, show: bool = False):
    ...
    # Panel 2: 1D Profile Showing Ambient Air to Surface Jumps
    ...
    if save_path:
        ...
        print(f"Convection wall plot saved to: {save_path}")
